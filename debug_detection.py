...
from colony_analysis.core import ColonyDetector, SAMModel
...
def debug_sam_detection(image_path, output_dir="debug_output"):
    ...
    # 可视化结果
    create_debug_visualization(img_rgb, masks_data, stable_masks, colonies, output_dir)
...
def create_debug_visualization(
    img_rgb, raw_masks, filtered_masks, colonies, output_dir
):
    """创建调试可视化"""
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/debug_visualization.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def save_debug_info(raw_masks, filtered_masks, colonies, output_dir):
    ...
    if colonies:
        ...
        for i, colony in enumerate(colonies):
            info_lines.append(
                f"  菌落{i+1}: ID={colony['id']}, 面积={colony['area']:.0f}, "
